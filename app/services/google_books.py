...
def get_best_match(query_title: str, query_author: str, api_results: list) -> dict:
    ...
    for item in api_results:
        ...
        if query_author and api_authors:
            # Find the best author match among the list of authors
            ...
            # If no author is queried, we don't penalize for it
    ...
    # Only return a match if it's a confident one
    if highest_score > 0.8:  # 80% confidence threshold
        logger.info(f"Found a confident match with score: {highest_score:.2f}")
        return best_match
    
    logger.warning(f"No confident match found. Highest score was {highest_score:.2f}")
    return None
...
def enrich_book_metadata(title: str, author: str, series_name: str, crop_name: str):
    ...
    try:
        ...
        if series_name and series_name.strip():
            query_parts.append(f'"{series_name.strip()}"')
            
        gb_query = "+".join(query_parts)
        logger.info(f"[GOOGLE_BOOKS] Query for {crop_name}: {gb_query}")
        ...
        if gb_data.get("items"):
            ...
            if best_match_info:
                # Extract and process data from the best match
                ...
                enrichment.update({
                    ...
                    "genre": unique_genres[:5],  # Limit to 5 genres
                    "isbn": isbn_list,
                    "author_from_api": author_from_api
                })
        else:
            logger.warning(f"[GOOGLE_BOOKS] No search results for {crop_name}")
